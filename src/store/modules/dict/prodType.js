...
export default {
  ...
  getters: {
    ...
    perMerge: (_state, _getters, _rootState, _rootGetters) => {
      const arr = _rootGetters["userRoleAuth/pageRoleAuth"](
        "h5_config_dic_prodType"
      );
      return arr.includes("h5_config_prodType_merge");
    },
    perPersist: (_state, _getters, _rootState, _rootGetters) => {
      const arr = _rootGetters["userRoleAuth/pageRoleAuth"](
        "h5_config_dic_prodType"
      );
      return arr.includes("h5_config_prodType_persist");
    },
  ...
  actions: {
    ...
    dataMerge: async (args0, val = {}) => {
      return prodTypeMerge(val);
    },
    dataPersist: async (args0, val = {}) => {
      return prodTypePersist(val);
    }
  }
};
