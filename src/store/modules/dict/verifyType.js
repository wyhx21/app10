...
export default {
  ...
  getters: {
    ...
    perMerge: (_state, _getters, _rootState, _rootGetters) => {
      const arr = _rootGetters["userRoleAuth/pageRoleAuth"](
        "h5_config_dic_verifyType"
      );
      return arr.includes("h5_config_verifyType_merge");
    },
    perPersist: (_state, _getters, _rootState, _rootGetters) => {
      const arr = _rootGetters["userRoleAuth/pageRoleAuth"](
        "h5_config_dic_verifyType"
      );
      return arr.includes("h5_config_verifyType_persist");
    },
  ...
  actions: {
    ...
    dataMerge: async (args0, val = {}) => {
      return verifyTypeMerge(val);
    },
    dataPersist: async (args0, val = {}) => {
      return verifyTypePersist(val);
    }
  }
};
