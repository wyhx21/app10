  ...
  <app-page-container>
    <div class="app-data-detail">
      <table style="width:100%">
        <tr>
          <td class="app-table-lable4">类型编码</td>
          <td>{{ currentData["dicKey"] }}</td>
        </tr>
        <tr>
          <td class="app-table-lable4">类型名称</td>
          <td>{{ currentData["dicValue"] }}</td>
        </tr>
        <tr>
          <td class="app-table-lable4">排序</td>
          <td>
            {{ currentData["dicSort"] }}
          </td>
        </tr>
...
export default {
  ...
  computed: {
    ...mapGetters("appDict/verifyType", ["currentData", "perMerge"])
  },
  methods: {
    editorData() {
      this.$router.replace("/dict/verifyTypeEditor");
    }
  }
};
</script>
